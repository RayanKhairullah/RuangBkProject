<x-layouts.app :title="__('Edit Catatan Prilaku')">
    <div class="max-w-2xl mx-auto my-10">
        <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-lg p-8 border border-indigo-100 dark:border-indigo-800">
            <h1 class="text-3xl font-bold text-orange-500 dark:text-orange-300 mb-2 text-center">{{ __('Edit Catatan Prilaku') }}</h1>
            <p class="text-gray-500 dark:text-gray-300 mb-6 text-center">Silakan edit data catatan siswa dengan lengkap dan benar.</p>

            <form action="{{ route('catatans.update', $catatan->id) }}" method="POST" class="space-y-5">
                @csrf
                @method('PUT')
                ...
                <div>
                    ...
                    <input type="text" name="nama_siswa" id="nama_siswa"
                        class="w-full px-4 py-2 rounded-full bg-indigo-50 dark:bg-gray-700 text-sm outline-none focus:ring-2 focus:ring-orange-300 transition text-gray-900 dark:text-gray-100"
                        placeholder="{{ __('Masukkan Nama Siswa') }}" value="{{ old('nama_siswa', $catatan->nama_siswa) }}" required>
                </div>
                ...
                <div>
                    ...
                    <input type="text" name="guru_pembimbing" id="guru_pembimbing"
                        class="w-full px-4 py-2 rounded-full bg-indigo-50 dark:bg-gray-700 text-sm outline-none focus:ring-2 focus:ring-orange-300 transition text-gray-900 dark:text-gray-100"
                        placeholder="{{ __('Masukkan Nama Guru Pembimbing') }}" value="{{ old('guru_pembimbing', $catatan->guru_pembimbing) }}" required>
                </div>

                <!-- Pilih Room -->
                <div>
                    <label for="room_id" class="block text-sm font-medium text-gray-700 dark:text-gray-200 mb-1">{{ __('Pilih Kelas') }}</label>
                ...
                <div>
                    ...
                    <input type="text" name="kasus" id="kasus"
                        class="w-full px-4 py-2 rounded-full bg-indigo-50 dark:bg-gray-700 text-sm outline-none focus:ring-2 focus:ring-orange-300 transition text-gray-900 dark:text-gray-100"
                        placeholder="{{ __('Jelaskan Kasus/Masalah') }}" value="{{ old('kasus', $catatan->kasus) }}" required>
                </div>
                ...
                <div>
                    ...
                    <input type="date" name="tanggal" id="tanggal"
                        class="w-full px-4 py-2 rounded-full bg-indigo-50 dark:bg-gray-700 text-sm outline-none focus:ring-2 focus:ring-orange-300 transition text-gray-900 dark:text-gray-100"
                        value="{{ old('tanggal', $catatan->tanggal) }}" required>
                </div>
                ...
                <div>
                    ...
                    <textarea name="catatan_guru" id="catatan_guru"
                        ...
                        rows="4" placeholder="{{ __('Tulis catatan guru...') }}" required>{{ old('catatan_guru', $catatan->catatan_guru) }}</textarea>
                </div>
                ...
                <div>
                    ...
                    <input type="number" name="poin" id="poin"
                        class="w-full px-4 py-2 rounded-full bg-indigo-50 dark:bg-gray-700 text-sm outline-none focus:ring-2 focus:ring-orange-300 transition text-gray-900 dark:text-gray-100"
                        min="1" max="100" placeholder="{{ __('Masukkan Poin (1-100)') }}" value="{{ old('poin', $catatan->poin) }}" required>
                    @error('poin')
